...
import numpy as np
import pandas as pd
from scipy.stats import shapiro, ttest_ind, mannwhitneyu, wilcoxon, t
...
def wilcoxon_paired_test(
    ...
    # Paired Wilcoxon signed-rank test for pre/post measurements.
    x = pd.Series(x).reset_index(drop=True)
    y = pd.Series(y).reset_index(drop=True)
    mask = x.notna() & y.notna()
    x_clean, y_clean = x[mask].to_numpy(), y[mask].to_numpy()
    w_stat, p_value = wilcoxon(x_clean, y_clean)
    p_label = "Wilcoxon signed-rank: p < 0.001" if p_value < 0.001 else f"Wilcoxon signed-rank: p = {p_value:.3f}"
    return {"statistic": w_stat, "p_value": p_value, "p_label": p_label, "n": int(mask.sum())}


def smd_continuous(x: pd.Series, y: pd.Series) -> float:
    # Standardized mean difference (Cohen's d with pooled SD) for two continuous groups.
...
def smd_binary(p1: float, p2: float) -> float:
    # Standardized mean difference for two proportions (Cohen's h variant with pooled SE).
