...
def diagnosis_codes_completion(df: pd.DataFrame, 
                               official_codes: pd.DataFrame,
                               df_code_col: str = 'SKSCode', 
                               df_desc_col: str = 'DiagnosisName', 
                               official_code_col: str = 'Kode',
                               official_desc_col: str = 'Tekst',
                               threshold: int = 89) -> pd.DataFrame:
    # Preprocess the diagnosis table in order to fix missing SKSCodes and shorten them into broader categories.
    start_time = time.time()
    print("Number of df observations in the df table: ", len(df))
    print("Columns in the df table: ", df.columns.tolist())
    ...
    for index, row in df.iterrows():
        ...
        if pd.isna(row[df_code_col]) or row[df_code_col] == 'nan':
            description = row[df_desc_col]
            ...
            if not code:
                print(f"Code in '{df_desc_col}' NOT FOUND - Looking for a code through fuzzy search...")
                result = process.extractOne(cleaned_description, official_codes[official_desc_col].to_list())
                if result:
                    best_match, score = result
                    if score > threshold:
                        print(f"Found a match through fuzzy search: {best_match} with -score: {score}")
                        code = official_codes[official_codes[official_desc_col] == best_match][official_code_col].values[0]

            # Update the SKSCode in the DataFrame 'd' directly at the current index
            if code:
                df.at[index, df_code_col] = code

    # Shorten the SKSCode into a 4 digit code
    df['SKSCode_short'] = df[df_code_col].str.extract(r'^([A-Za-z]{2}\d{2})')
