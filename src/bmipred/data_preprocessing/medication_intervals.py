...
def filter_medication_timestamps(df: pd.DataFrame,
                                     start_col: str = "StartInstant",
                                     discontinued_col: str = "DiscontinuedInstant",
                                     administration_col: str = "AdministrationInstant",
                                     date_lo: pd.Timestamp = pd.Timestamp("2016-01-01"),
                                     date_hi: pd.Timestamp = pd.Timestamp("2026-01-01"),
                                     ongoing_fill_date: pd.Timestamp = pd.Timestamp("2025-12-31 23:59:59.999999999"),
                                     ) -> pd.DataFrame:
                                 
    for col in [start_col, discontinued_col, administration_col]:
        df[col] = pd.to_datetime(df[col], errors='coerce')

    df = df.dropna(subset=[start_col])
    print("Rows after dropping missing StartInstant:", len(df))

    # Label sources before any date manipulation
    df["HasAdministrationRecord"] = df[administration_col].notna()
    df["HasPrescriptionRecord"] = ~df["HasAdministrationRecord"]
    df["AdministrationRecordCount"] = df["HasAdministrationRecord"].astype(np.int32)
    df["PrescriptionRecordCount"] = df["HasPrescriptionRecord"].astype(np.int32)
    df["PossiblyOngoingMedication"] = (df[discontinued_col].isna() & df[administration_col].isna())

    # Fill DiscontinuedInstant: keep existing or replace with AdministrationInstant or with ongoing_fill_date
    df[discontinued_col] = (
        df[discontinued_col]
        .combine_first(df[administration_col])
        .fillna(ongoing_fill_date)
    )

    # If DiscontinuedInstant < StartInstant, set StartInstant = DiscontinuedInstant
    error_mask = df[discontinued_col] < df[start_col]
    df.loc[error_mask, start_col] = df.loc[error_mask, discontinued_col]

    # Filter to valid date range [date_lo, date_hi)
    df = df[
        df[start_col].between(date_lo, date_hi, inclusive='left') &
        (
            df[discontinued_col].isna() |
            df[discontinued_col].between(date_lo, date_hi, inclusive='left')
        )
    ]
...
def medication_intervals(df:pd.DataFrame,
    ...
    while True:
        ...
        duration_delta = df[discontinued_col] - df[start_col]
        df["MedicationDuration"] = duration_delta.dt.days
        df.loc[
            (duration_delta > pd.Timedelta(0)) & (df["MedicationDuration"] == 0),
            "MedicationDuration"
        ] = 1
        ...
        df = df.groupby([patientid_col, 'GroupNumber']).agg({
            ...
            'HasAdministrationRecord': 'max',
            'HasPrescriptionRecord': 'max',
            'AdministrationRecordCount': 'sum',
            'PrescriptionRecordCount': 'sum',
            'PossiblyOngoingMedication': 'max',
        ...
        df['MedicationIntervalSource'] = np.select(
            [
                df['HasAdministrationRecord'] & df['HasPrescriptionRecord'],
                df['HasAdministrationRecord'],
                df['HasPrescriptionRecord'],
            ],
            ['both', 'administration', 'prescription'],
            default='unknown',
        )
