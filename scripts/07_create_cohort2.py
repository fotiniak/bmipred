...
def main():
    ...
    # Cohort 2: Olanzapine initiation with follow-up 30-180 days after start
    ...
    cohort2 = medication[medication["ATC"]=="N05AH03"]
    print("Number of individuals treated with olanzapine:", cohort2.PatientDurableKey.nunique(), cohort2.shape)
    
    # Add past BMI - health assessment features
    health_assessment = pd.read_parquet(TABLES_TO_COMBINE["health_assessment"]["input_path"])
    
    # Merge the BMI information:
    
    cohort2 = pd.merge_asof(cohort2.sort_values(by=[TABLES_TO_COMBINE["medication"]["column_names"]["start_col"]], ascending=[True], na_position="last"),
    ...
    cohort2["bmi_days_after_medication_start"] = cohort2[TABLES_TO_COMBINE["health_assessment"]["column_names"]["instant_col"]] - cohort2[TABLES_TO_COMBINE["medication"]["column_names"]["start_col"]]
    cohort2["bmi_days_before_medication_stop"] = cohort2[TABLES_TO_COMBINE["medication"]["column_names"]["end_col"]] - cohort2[TABLES_TO_COMBINE["health_assessment"]["column_names"]["instant_col"]]
    cohort2 = cohort2[cohort2["bmi_days_before_medication_stop"] >= pd.Timedelta("0D")] # This way we will remove NAs and BMI measurements that occur after N05AH03 discontinuation
    cohort2 = cohort2[cohort2["bmi_days_after_medication_start"] >= pd.Timedelta("30D")] # 30D wash out period 
    
    # Just to check how many observations we can have for 30-180 days follow up:
    print("Amount of observations for on olanzapine with follow up:", cohort2.shape)
    print("Amount of unique individuals for on olanzapine with follow up:", cohort2[TABLES_TO_COMBINE["medication"]["column_names"]["patientid_col"]].nunique())

    print("Amount of observations for 30-180 days follow up:", cohort2.loc[cohort2["bmi_days_after_medication_start"].between(pd.Timedelta("30D"), pd.Timedelta("180D"), inclusive="both")].shape)
    print("Number of unique individuals for 30-180 days follow up:", cohort2.loc[cohort2["bmi_days_after_medication_start"].between(pd.Timedelta("30D"), pd.Timedelta("180D"), inclusive="both")]["PatientDurableKey"].nunique())

    # Add medication info - ATC codes up to olanzapine initiation.

    cohort2 = pd.merge_asof(cohort2.sort_values(by=[TABLES_TO_COMBINE["health_assessment"]["column_names"]["instant_col"]], ascending=[True], na_position="last"),
                            medication.sort_values(by=[TABLES_TO_COMBINE["medication"]["column_names"]["start_col"]], ascending=[True], na_position="last"),
                            left_on= [TABLES_TO_COMBINE["health_assessment"]["column_names"]["instant_col"]],
                            right_on= [TABLES_TO_COMBINE["medication"]["column_names"]["start_col"]],
                            by=[TABLES_TO_COMBINE["health_assessment"]["column_names"]["patientid_col"]],
                            direction="backward", # most recent previous row
                            allow_exact_matches=True,
                            suffixes=("_olanzapine", "")) # including at olanzapine start time
    ...
    lab_results = lab_results[lab_results[TABLES_TO_COMBINE["lab_results"]["column_names"]["patientid_col"]].isin(cohort2[TABLES_TO_COMBINE["health_assessment"]["column_names"]["patientid_col"]].unique())]
    ...
    cohort2 = cohort2.merge(lab_results_history,on=[TABLES_TO_COMBINE["lab_results"]["column_names"]["patientid_col"], TABLES_TO_COMBINE["lab_results"]["column_names"]["instant_col"]],how="left")
    ...
    hospitalization = hospitalization[hospitalization[TABLES_TO_COMBINE["hospitalization"]["column_names"]["patientid_col"]].isin(cohort2[TABLES_TO_COMBINE["health_assessment"]["column_names"]["patientid_col"]].unique())]
    ...
    cohort2["is_currently_hospitalized"] = (
        (cohort2[TABLES_TO_COMBINE["health_assessment"]["column_names"]["instant_col"]] >= cohort2[TABLES_TO_COMBINE["hospitalization"]["column_names"]["start_col"]]) &
    (
        (cohort2[TABLES_TO_COMBINE["hospitalization"]["column_names"]["start_col"]] <= cohort2[TABLES_TO_COMBINE["hospitalization"]["column_names"]["end_col"]]) |
        (cohort2[TABLES_TO_COMBINE["hospitalization"]["column_names"]["end_col"]].isna())
    )
    ).astype("Int8")
    ...
    cohort2 = cohort2.sort_values(by=[TABLES_TO_COMBINE["health_assessment"]["column_names"]["instant_col"]], ascending=[True], na_position="last").drop_duplicates(subset=[TABLES_TO_COMBINE["health_assessment"]["column_names"]["patientid_col"]], keep='first')
    
    # Save the cohort table
    cohort2.to_parquet(BASE_DIR / "cohort2_olanzapine_on_treatment.parquet", index=False)

    # -----------------------------------------------
    # Preprocessing - Refinement of the cohort table.
    # -----------------------------------------------

    cohort2 = pd.read_parquet(BASE_DIR / "cohort2_olanzapine_on_treatment.parquet")

    # --- Remove lab tests that are >50% missing ---
    
    cohort2_lab_tests = [col for col in cohort2.columns if col.startswith('Labanalysis')]
    na_frequencies = cohort2[cohort2_lab_tests].isna().mean()
    columns_to_remove = na_frequencies[na_frequencies > 0.5].index
    cohort2 = cohort2.drop(columns=columns_to_remove)
    ...
    cohort2.replace('None', np.nan, inplace=True)
    cohort2.replace({None: np.nan}, inplace=True)
    
    # Convert True/False or "True"/"False" to 1/0
    # Real boolean columns -> nullable integer
    bool_cols = cohort2.select_dtypes(include=["bool", "boolean"]).columns
    cohort2[bool_cols] = cohort2[bool_cols].astype("Int8")   # True/False -> 1/0, keeps NA
    # String booleans in object/string columns
    obj_cols = cohort2.select_dtypes(include=["object", "string"]).columns
    cohort2[obj_cols] = cohort2[obj_cols].replace({"True": 1, "False": 0})
    ...
    available = [c for c in cols_to_encode if c in cohort2.columns]
    
    if available:
        dummies = pd.get_dummies(cohort2[available], prefix=available, drop_first=False)
        cohort2 = pd.concat([cohort2.drop(available, axis=1), dummies], axis=1)
    
    # Convert all 0/1 Columns to Int8
    for col in cohort2.columns:
        if pd.api.types.is_numeric_dtype(cohort2[col]):
            unique_vals = cohort2[col].dropna().unique()
            if set(unique_vals).issubset({0,1}):
                cohort2[col] = cohort2[col].astype("Int8")
    ...
    if "active_atc_codes_csv" in cohort2.columns:
        cohort2["active_atc_codes_csv_short"] = (
            cohort2["active_atc_codes_csv"]
            .fillna("")
            .astype(str)
            .str.split(",")
    ...
    if "active_sks_codes_csv" in cohort2.columns:
        cohort2["active_sks_codes_csv_short"] = (
            cohort2["active_sks_codes_csv"]
            .fillna("")
            .astype(str)
            .str.split(",")
            .apply(lambda xs: ",".join(x.strip()[:4] for x in xs if x.strip()))
    ...
    # For ATCs:
    code_counts_atc = cohort2[atcs].str.split(',', expand=True).stack().value_counts()
    top_atc_codes = code_counts_atc.nlargest(200).index.tolist()
    
    cohort2['ATCs_filtered'] = cohort2[atcs].apply(lambda x: ','.join(
        c.strip() for c in x.split(',') 
        if c.strip() in top_atc_codes
    ) if pd.notnull(x) else '')
    
    atc_dummies = cohort2['ATCs_filtered'].str.get_dummies(sep=',').add_prefix('ATC_')
    cohort2 = pd.concat([cohort2, atc_dummies], axis=1)
    
    # For SKSCodes:
    code_counts_sks = cohort2[skscodes].str.split(',', expand=True).stack().value_counts()
    top_sks_codes = code_counts_sks.nlargest(200).index.tolist()
    
    cohort2['SKSCodes_filtered'] = cohort2[skscodes].apply(lambda x: ','.join(
        c.strip() for c in x.split(',') 
        if c.strip() in top_sks_codes
    ) if pd.notnull(x) else '')
    
    sks_dummies = cohort2['SKSCodes_filtered'].str.get_dummies(sep=',').add_prefix('SKS_')
    cohort2 = pd.concat([cohort2, sks_dummies], axis=1)
    ...
    for col in cohort2.columns:
        if pd.api.types.is_numeric_dtype(cohort2[col]):
            unique_vals = cohort2[col].dropna().unique()
            if set(unique_vals).issubset({0,1}):
                cohort2[col] = cohort2[col].astype("boolean")

    # --- Calculate the target variable ---
    # Make sure the target variable is calculates correctly
    
    cohort2["target"] = ((cohort2["bmi_diff_from_next"] >= cohort2["BodyMassIndex_recalc"] * 0.05).astype("int8")) # BMi increase >5%
    #cohort2["target"] = (cohort2["bmi_diff_from_next"] > 2).astype("int8") # BMI change more than 2 units
    #cohort2["target"] = cohort2["bmi_diff_from_next"] # Exact BMI change
    ...
    cols_to_drop = [
        ...
        'active_sks_codes_csv_short',
        'past_atc_code_treatments',
        "bmi_days_before_medication_stop",
        ]

    existing = [c for c in cols_to_drop if c in cohort2.columns]
    cohort2 = cohort2.drop(columns=existing)
    cohort2 = cohort2.loc[:, (~cohort2.columns.str.endswith('_olanzapine')) | (cohort2.columns == 'past_atc_code_treatments_olanzapine')] # keep _history columns and past_atc_code_treatments_olanzapine from first merge, drop other _olanzapine

    col_list = cohort2.columns.tolist()   # get every column name as a Python list
    print("cohort2.shape:", cohort2.shape, cohort2["PatientDurableKey"].nunique())
    print(col_list)

    # Save the final cohort table for modeling
    cohort2.to_parquet(BASE_DIR / "cohort2_olanzapine_on_treatment_final.parquet", index=False)
